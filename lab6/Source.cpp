...
результатов предоставляется временный файл в текущем каталоге. */
{
	...
	SECURITY_ATTRIBUTES StdOutSA = { sizeof
	(SECURITY_ATTRIBUTES), NULL, TRUE };/* права доступа для наследуемого дескриптора. */
	...
	for (iProc = 0; iProc < argc - 2; iProc++)
	{
		...
		sprintf_s(CommandLine, _T("%s%s%s"),
	...
	for (iProc = 0; iProc < argc - 2; iProc++)
	{
		...
		if (GetExitCodeProcess(hProc[iProc], &ExCode) && ExCode == 0)
		{
			...
			sprintf_s(outFile, _T("%s"), ProcFile[iProc].TempFile);
			cat(argv[iProc + 2], (LPTSTR)outFile);
			sprintf_s(CommandLine, _T("%s%s"),
